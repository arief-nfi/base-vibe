  ...
  if (!isAuthorized([], ['master.webhook.view'])) {
    return (
      <div className="flex items-center justify-center h-96">
        <p className="text-gray-500">You don't have permission to view webhook details.</p>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <Button variant="outline" onClick={() => navigate('/console/master/integration-outbound/webhook')}>
            <ArrowLeft className="mr-2 h-4 w-4" />
            Back to List
          </Button>
          {isAuthorized([], ['master.webhook.edit']) && (
            <Button onClick={() => navigate(`/console/master/integration-outbound/webhook/edit/${webhook.id}`)}>
      ...
      <div className="grid gap-6 md:grid-cols-2">
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            <div>
              <label className="text-sm font-medium text-gray-500">Webhook ID</label>
              <p className="text-sm font-mono bg-gray-50 p-2 rounded border">{webhook.id}</p>
            </div>
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            <div>
              ...
              <p className="text-sm">{webhook.partnerName || 'Unknown Partner'}</p>
            </div>
            
            <div>
              <label className="text-sm font-medium text-gray-500">Partner Code</label>
              <p className="text-sm font-mono">{webhook.partnerCode || 'N/A'}</p>
            </div>
            
            <div>
              <label className="text-sm font-medium text-gray-500">Partner ID</label>
              <p className="text-sm font-mono bg-gray-50 p-2 rounded border">{webhook.partnerId}</p>
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="md:col-span-2">
          <CardHeader>
            ...
            <CardDescription>URL where events will be delivered</CardDescription>
          </CardHeader>
          <CardContent>
        ...
        <Card className="md:col-span-2">
          <CardHeader>
            ...
            <CardDescription>Webhook creation and modification history</CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <p className="text-sm">{new Date(webhook.updatedAt).toLocaleString()}</p>
              </div>
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="md:col-span-2">
          <CardHeader>
            ...
            <CardDescription>Details about the monitored event type</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="space-y-4">
              <div>
                <h4 className="text-sm font-medium mb-2">Event Type: {webhook.eventType}</h4>
...
function getEventDescription(eventType: string): string {
  const descriptions: Record<string, string> = {
    ...
    'integration.key.created': 'Triggered when a new API integration key is generated',
  ...
  return descriptions[eventType] || 'Custom event type for specialized notifications';
}
